...
const { SimpleLayout } = require('./graphPhysicsEngine');
...
const E = {
  ...
  btnSemantic: g('btnSemantic'), btnHealth: g('btnHealth'), btnLabels: g('btnLabels'), btnHoverFocus: g('btnHoverFocus'),
...
// stays byte-identical to the one real copy of the class, instead of drifting
// like the old hand-typed duplicate did.
...
function _clientBodyRendering() {
  ...
  S.renderer.setHoverFocus(hoverFocusEnabled);
...
function toggleHoverFocus() {
  ...
  if (S.renderer) S.renderer.setHoverFocus(hoverFocusEnabled);
  if (E.btnHoverFocus) {
    E.btnHoverFocus.classList.toggle('on', hoverFocusEnabled);
    E.btnHoverFocus.setAttribute('aria-pressed', hoverFocusEnabled ? 'true' : 'false');
  }
}
...
if (E.btnHoverFocus) E.btnHoverFocus.addEventListener('click', toggleHoverFocus);
