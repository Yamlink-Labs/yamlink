...
function buildCalendarModel(taskRows, fieldsCache, todayIso) {
    ...
    const dated = rows
        .filter(row => row.date)
        .sort((a, b) => a.date.localeCompare(b.date) || a.fileId.localeCompare(b.fileId) || a.id.localeCompare(b.id));
    const undated = rows
        .filter(row => !row.date)
        .sort((a, b) => a.fileId.localeCompare(b.fileId) || a.id.localeCompare(b.id));
...
function renderCalendar() {
    ...
    try {
        ...
        host.webview.html = buildHtml(model, nonce, csp);
    } catch (error) {
...
function buildHtml(model, nonce, csp) {
...
function renderSummary() {
  ...
  const taskRows = rows.filter(row => row.itemKind !== 'created');
  ...
  agendaList.innerHTML = rows.map(row => {
    return '<article class="task' + (row.done ? ' done' : '') + '">' +
      ...
      '<div class="task-main">' +
        ...
        '<div class="task-meta">' +
          '<span class="pill">' + (row.itemKind === 'created' ? 'created' : (row.done ? 'done' : 'open')) + '</span>' +
