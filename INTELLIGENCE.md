...
When the arc has nothing confident to say, the cascade now falls back to **workflow memory** (`src/intelligence/workflowMemory.js`) — a real, repeated pattern of *you* filling two fields together on notes of this type (e.g. always adding `phone` shortly after `email` on `contact` notes), detected from real authoring sessions (field edits on the same note within a 15-minute window) rather than a schema or vault-frequency signal. Gated by the same kind of confidence bar as everything else here (60%+ co-occurrence across at least 2 real sessions), and it never overrides the arc-based suggestion — it only fills the gap when the arc is silent. Inspect the raw patterns directly with `yamlink workflow-memory --type <type>`.
